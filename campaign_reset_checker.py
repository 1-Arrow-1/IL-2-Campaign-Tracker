...
def cleanup_popups_for_reset_campaigns() -> bool:
    ...
    if not os.path.exists(decoded_path):
        log_message(logger, "[WARN] campaigns_decoded.json not found at", decoded_path)
        return False
    ...
    try:
        ...
        log_message(logger, "[WARN] campaigns_decoded.json invalid JSON")
        return False

    if not os.path.exists(popups_path):
        log_message(logger, "[INFO] campaign_popups_seen.json not found – skipping cleanup")
        return False
    ...
    if not reset_campaigns: 
        log_message(logger, "[INFO] No reset campaigns detected.")
        return False

    log_message(
        logger,
        f"[INFO] Found {len(reset_campaigns)} reset campaign(s): {', '.join(reset_campaigns)}",
    )
    modified = False
    ...
    if os.path.exists(events_path):
        try:
            ...
            log_message(logger, "[WARN] campaign_events.json invalid JSON – skipping cleanup")
            events_data = None
        ...
        if isinstance(events_data, dict):
            ...
            for campaign_name in reset_campaigns:
                if events_data.get(campaign_name) != []:
                    ...
                    log_message(logger, f"  🧹 Cleared events: '{campaign_name}'")
            ...
            if events_modified:
                ...
                log_message(logger, "[INFO] campaign_events.json updated")
                modified = True
    else:
        log_message(logger, "[INFO] campaign_events.json not found – skipping cleanup")
    ...
    for campaign_name in reset_campaigns:
        if campaign_name in popups_data:
            ...
            log_message(logger, f"  🧹 Removed popups: '{campaign_name}'")
            modified = True

    if modified:
        save_popup_seen(popups_path_obj, popups_data)
        log_message(logger, "[INFO] campaign_popups_seen.json updated")
    ...
    if os.path.exists(reports_dir):
        for campaign_name in reset_campaigns:
            ...
            if os.path.exists(pdf_path):
                try:
                    os.remove(pdf_path)
                    log_message(logger, f"  🗑️  Deleted PDF: {safe_name}_Report.pdf")
                    modified = True
    ...
    # ================================================================
    if modified:
        log_message(logger, "[INFO] ✅ Reset campaign cleanup complete")
        return True
    else:
        log_message(logger, "[INFO] No cleanup actions needed")
        return False
