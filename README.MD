...
4. **Extract** Campaigns.gtp (standard IL-2 BoS campaigns) from `<Path to Il-2 BoS>\data` with the help of unGTP-IL2 (download it here https://www.mediafire.com/file/caxpalaudz1hd47/unGTP-IL2.zip). Put the `unGTP-IL2.exe` in `<Path to Il-2 BoS>\data`.
   ...
   **Copy** the campaigns folder to `<Path to Il-2 BoS>\data`.   
   ⚠️**IMPORTANT** Only campaigns in `<Path to Il-2 BoS>\data\campaigns` can be monitored by the tracker.
